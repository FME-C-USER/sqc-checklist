...
(function (root) {
  ...
  // 店號補回前導0並以「文字」輸出（Sheet 常把 022320 存成數字 22320，
  // 若以數字寫進 Excel 會顯示成 22,320 而掉0）
  function padStoreCode(c) {
    var s = String(c == null ? '' : c).trim();
    return /^\d+$/.test(s) && s.length < 6 ? ('000000' + s).slice(-6) : s;
  }
